...
| `/home/vasantha/AI-Projects/ClariFin_OS/backend/src/db.py`                | Wraps `core.db.connection.get_connection()`                                                 | Yes           | ✅ **Canonical**. Backward-compatible wrapper for schema/migration connections.                |
...
| `/home/vasantha/AI-Projects/ClariFin_OS/backend/src/engines/reconciliation_engine.py` | Direct `sqlite3.connect()` (Lines 282, 335)                                                 | No            | ❌ **Violation**. Must use `repositories/base.py` or `core.db.connection.get_connection()`.   |
| `/home/vasantha/AI-Projects/ClariFin_OS/backend/src/engines/ledger_audit_engine.py` | Direct `sqlite3.connect()` (Lines 36, 156)                                                  | No            | ❌ **Violation**. Must use `repositories/base.py` or `core.db.connection.get_connection()`.   |
| `/home/vasantha/AI-Projects/ClariFin_OS/backend/src/engines/behavior_engine.py` | Direct `sqlite3.connect()` (Lines 121, 150, 188, 228, 289, 314)                            | No            | ❌ **Violation**. Must use `repositories/base.py` or `core.db.connection.get_connection()`.   |
| `/home/vasantha/AI-Projects/ClariFin_OS/backend/src/engines/balance_engine.py` | Direct `sqlite3.connect()` (Lines 97, 175, 233, 282)                                        | No            | ❌ **Violation**. Must use `repositories/base.py` or `core.db.connection.get_connection()`.   |
...
- ✅ **All PRAGMAs enforced**:
  ...
  - `busy_timeout` (not explicitly set, but recommended to add).
  - Isolation level: Default (autocommit off).
