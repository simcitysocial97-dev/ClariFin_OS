...
from runtime.platform.ai.context.provenance import ProvenanceTracker, PROVENANCE_TRACKER_INSTANCE
from runtime.platform.ai.context.ranker import RankedComponent
from runtime.platform.ai.context.serializer import compute_pack_id, serialize_context_pack, estimate_tokens
from runtime.platform.ai.context.trimmer import Trimmer, TRIMMER_INSTANCE
...
class ContextBuilder:
    ...
    def build(
        ...
        *,
        ...
        pack_data = {
            ...
            ] if result.omitted_components else [],
    ...
    def _gather_failing_evidence(self, capability_id: str | None, run_id: str | None) -> list[RankedComponent]:
            ...
            for event in store.iter_events():
                ...
                if not failed:
                    ref = self.provenance.add(
                        type(event).__name__,
                        event.event_id,
                        path=event.execution_context.get("path") if event.execution_context else None,
                    )
                    comps.append(RankedComponent(
                        component_type="failing_evidence",
                        source_kind="evidence",
                        data={"event_id": event.event_id, "timestamp": event.timestamp.isoformat()},
                        provenance_ref=ref,
    ...
    def _gather_recent_change(self) -> list[RankedComponent]:
        ...
        comps: list[RankedComponent] = []
        try:
            from runtime.foundation.verification.change_surface import discover_working_tree_changes
            files = discover_working_tree_changes()
            for path in files[:5]:  # top 5 changed files
                ref = self.provenance.add("repository", path, path=path)
                comps.append(RankedComponent(
                    component_type="recent_change",
                    source_kind="repository",
                    data={"path": path, "change_type": "modified"},
                    provenance_ref=ref,
                    token_estimate=estimate_tokens(path),
                    relevance_score=80,
                ))
        except Exception as exc:
    ...
    def _gather_capability(self, capability_id: str | None) -> list[RankedComponent]:
        ...
        try:
            ...
            if detail:
                ref = self.provenance.add("knowledge", capability_id)
                data = detail["data"]
                comps.append(RankedComponent(
    ...
    def _gather_recent_run(self, run_id: str | None, capability_id: str | None) -> list[RankedComponent]:
        ...
        try:
            ...
            for event in store.iter_events():
                ...
                if capability_id and meta.get("capability_id") != capability_id:
                    continue
                ref = self.provenance.add("history", event.event_id)
                comps.append(RankedComponent(
    ...
    def _gather_knowledge(self, capability_id: str | None, intent_type: str) -> list[RankedComponent]:
        """Gather knowledge entries for the capability."""
        comps: list[RankedComponent] = []
        if not capability_id:
            return comps
        try:
            ref = self.provenance.add("knowledge", f"{capability_id}.intent")
            comps.append(RankedComponent(
    ...
    def _gather_adjacent_capabilities(self, capability_id: str | None) -> list[RankedComponent]:
        ...
        try:
            ...
            if graph:
                ...
                for cap_id in set(upstream + downstream):
                    ref = self.provenance.add("knowledge", f"{cap_id}.adjacent")
                    comps.append(RankedComponent(
    ...
    def _gather_architecture(self) -> list[RankedComponent]:
        ...
        try:
            ...
            if auth:
                ...
                for item in items:
                    ref = self.provenance.add("architecture", item.get("name", ""))
                    comps.append(RankedComponent(
