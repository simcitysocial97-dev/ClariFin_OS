...
def canonical_help() -> str:
    ...
    lines.append("  check        — primary verification entrypoint")
    lines.append("  plan         — plan-only mode (no execution)")
    lines.append("  run          — execute an explicit plan")
    lines.append("  diagnose     — failure/survivor diagnostic")
    lines.append("  strengthen   — test-strengthening control plane")
    lines.append("  inspect      — read-only inspection queries")
    lines.append("  certify      — certification (evidence-gated)")
    lines.append("  ci           — CI orchestration / reconciliation")
    lines.append("  doctor       — framework health / integrity")
    lines.append("")
