...
class ApiContractGate:
    ...
    def check_freshness(self) -> DimensionResult:
        ...
        return DimensionResult(name="freshness", status=status, failures=tuple(failures))
    ...
    def check_generated_types(self) -> DimensionResult:
        ...
        try:
            ...
            return DimensionResult(name="generated_types", status="skip", metadata={"reason": "cannot generate live OpenAPI"})

        # Generate to temp file using openapi-typescript
        with tempfile.NamedTemporaryFile(suffix=".ts", mode="w", delete=False, dir="/tmp") as tmp:
            tmp_path = tmp.name
        ...
        try:
            ...
            oapi_tmp = tempfile.NamedTemporaryFile(suffix=".json", mode="w", delete=False, dir="/tmp")
            ...
            if gen_proc.returncode == 0:
                ...
                self._inventory.generated_types_hash = hashlib.sha256(new_content.encode()).hexdigest()
                if new_content.strip() != current_content.strip():
                    ...
                    return DimensionResult(name="generated_types", status="fail", failures=tuple(failures))
                else:
    ...
    def check_schema_compatibility(self) -> DimensionResult:
        ...
        for zod_name, op_api_name in zod_to_openapi.items():
            zod_schema = next((s for s in inventory.runtime_schemas if s.name == zod_name), None)
            if not zod_schema:
                continue
            ...
            for field_name in openapi_props:
                if field_name not in zod_shape:
                    ...
                    is_nullable_openapi = "null" in str(schema_spec.get("type", "")) or any(
                        t.get("type") == "null" for t in schema_spec.get("anyOf", [])
                    ...
                    zod_min_val = zod_min[0] if isinstance(zod_min, list) and zod_min else zod_min
                    zod_max_val = zod_max[0] if isinstance(zod_max, list) and zod_max else zod_max
                    openapi_min = schema_spec.get("minimum")
                    openapi_max = schema_spec.get("maximum")
                    if zod_min_val is not None and openapi_min is not None and zod_min_val != openapi_min:
                        failures.append(_failure(
                    ...
                    if zod_max_val is not None and openapi_max is not None and zod_max_val != openapi_max:
                        failures.append(_failure(
                            FailureClassification.SCHEMA_DRIFT,
                            operation=f"schema:{zod_name}",
                            path=f"{op_api_name}.{field_name}",
                    ...
                    if zod_max_val is not None and zod_max_val >= 100 and openapi_max is None:
                        if "ratio" in op_desc.lower() or "(0-1)" in op_desc or "(0 – 1)" in op_desc:
                            failures.append(_failure(
                                FailureClassification.SCHEMA_DRIFT,
                                operation=f"schema:{zod_name}",
                                path=f"{op_api_name}.{field_name}",
            ...
            for field_name in zod_shape:
                if field_name not in openapi_props and field_name not in zod_schema.nullable_fields:
        ...
        status = "pass" if not failures else "fail"
        return DimensionResult(name="schema_compat", status=status, failures=tuple(failures))
    ...
    def check_consumer_integrity(self) -> DimensionResult:
        ...
        return DimensionResult(name="consumer_integrity", status=status, failures=tuple(failures))
    ...
    def check_wire_validation(self) -> DimensionResult:
        ...
        try:
            ...
            from runtime.foundation.verification.api_contracts.fixture import seed_contract_fixture
            ...
            for method, path in critical_endpoints:
                ...
                if path == "/api/transactions":
                    ...
                    for field_name in ("net_cash_flow_paise", "total_income_paise", "total_expenses_paise", "emi_paise"):
                        if field_name in data:
                            val = data[field_name]
