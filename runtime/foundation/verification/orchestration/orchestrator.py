"""M9-C49 — ExecutionOrchestrator and convenience functions.

Extracted from the monolithic execution_orchestrator.py (lines 528–2108).
"""
...
class ExecutionOrchestrator:
    ...
    def _detect_and_report_regressions(
        self,
        plan: ExecutionPlan,
        records: list[TaskExecutionRecord],
        live_fp,
    ) -> dict:
        """Extract metrics from execution records and detect regressions.

        Stores run metrics and compares against historical baseline.
        Logs warnings to stderr if regressions are detected.
        """
        from ..regression_detector import RegressionDetector, RunMetrics

        # Extract coverage and mutation scores from measurement truth records
        ...
        for rec in records:
            mt = rec.measurement_truth
            if mt is None:
                continue
            kind = mt.get("kind", "")
            score = mt.get("score")
            if kind == "coverage" and score is not None:
                coverage_pct = max(coverage_pct, float(score))
            elif kind == "mutation" and score is not None:
                mutation_score = max(mutation_score, float(score))

        # Also check record diagnostics for coverage/mutation mentions
        for rec in records:
            if rec.exit_code == 0:
                passed += 1
            else:
            ...
            if rec.verification_kind in ("coverage", "mutation"):
                test_count += 1

        if test_count == 0:
            test_count = passed + failed
        if passed == 0 and failed == 0:
            passed = len([r for r in records if r.exit_code == 0])
            failed = len([r for r in records if r.exit_code != 0])

        run_id = f"run-{plan.plan_id}-{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}"
        branch = "main"  # default; could be derived from git in production
        ...
        detector.store_run_metrics(run_id, branch, current_metrics.__dict__)

        baseline = detector.get_baseline_metrics(branch)
        if baseline is None:
            return {
        ...
        report = detector.detect_regressions(current_metrics, baseline)
        if report.alerts:
            print("\n⚠️  REGRESSION DETECTED:", file=sys.stderr)
        ...
        return {
            ...
            "mutation_delta": report.mutation_delta,
            "test_delta": report.test_delta,
