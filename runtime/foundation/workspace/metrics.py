...
def render_metrics(workspace: MetricsWorkspace) -> str:
    ...
    rows = [
        [
            "Local",
            str(workspace.local_verification.total_runs),
            format_percent(workspace.local_verification.success_rate),
        ],
        [
            "CI",
            str(workspace.ci_verification.total_runs),
            format_percent(workspace.ci_verification.success_rate),
        ],
        [
            "Combined",
            str(workspace.verification.total_runs),
            format_percent(workspace.verification.success_rate),
        ],
    ]
    ...
    lines.append(
        render_table(
            ...
            [
                [
                    "Local",
                    str(workspace.local_cache.hits),
                    str(workspace.local_cache.total),
                    format_percent(workspace.local_cache.hit_rate),
                ],
                [
                    "CI",
                    str(workspace.ci_cache.hits),
                    str(workspace.ci_cache.total),
                    format_percent(workspace.ci_cache.hit_rate),
                ],
                [
                    "Combined",
                    str(workspace.cache.hits),
                    str(workspace.cache.total),
                    format_percent(workspace.cache.hit_rate),
                ],
            ],
        )
    )
    ...
    lines.append(
        render_table(
            ...
            [
                [
                    "Local",
                    format_duration(workspace.local_duration.avg_seconds),
                    format_duration(workspace.local_duration.min_seconds),
                    format_duration(workspace.local_duration.max_seconds),
                ],
                [
                    "CI",
                    format_duration(workspace.ci_duration.avg_seconds),
                    format_duration(workspace.ci_duration.min_seconds),
                    format_duration(workspace.ci_duration.max_seconds),
                ],
                [
                    "Combined",
                    format_duration(workspace.duration.avg_seconds),
                    format_duration(workspace.duration.min_seconds),
                    format_duration(workspace.duration.max_seconds),
                ],
            ],
        )
    )
    ...
    rows = [
        [
            g.category,
            str(g.current_count),
            str(g.previous_count),
            str(g.delta),
            format_percent(g.growth_rate * 100) if g.growth_rate else "0.0%",
        ]
        for g in workspace.dependency_growth
    ]
    lines.append(
        render_table(["Category", "Current", "Previous", "Delta", "Growth Rate"], rows)
    )
