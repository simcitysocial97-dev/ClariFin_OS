...
def test_canonical_help_lists_only_nine_operations():
    ...
    expected_ops = ["check", "plan", "run", "diagnose", "strengthen", "inspect", "certify", "ci", "doctor"]
...
def test_no_duplicate_authority_in_classification():
    ...
    from runtime.foundation.verification.canonical_control_plane import classification_for

    tokens_to_test = [
        "status", "metrics", "history", "deps", "verify-status", "analytics",
        "health", "doctor", "ci-doctor", "diagnose", "diagnose-failures",
        "plan", "reconcile", "exec-evidence", "deep-contract", "local-gate",
        "affected", "repair", "risk", "integrity", "knowledge", "dashboard",
        "intelligence", "certify-v4", "certify-v5", "intelligence-audit",
        "audit", "api-contracts", "contract-governance", "mutation",
        "measurement-truth", "measurement", "evidence-plan",
        "verification-contract", "evidence-execute", "evidence-reconcile",
        "evidence-certify", "enforce", "mutation-inventory", "mutation-intel",
        "forensic-diagnose", "forensic-report", "strengthen-analyze",
        "strengthen-discover", "strengthen-propose", "strengthen-validate",
        "strengthen-survivor-forensic", "strengthen-report", "env-check",
        "env-contract", "what-should-i-run", "capability-inventory",
        "control-plane-plan", "resolve-capabilities", "strengthen-capability",
        "strengthen-survivor", "measurement-truth-report", "blast-radius",
        "execution-plan", "execute", "execution-status", "execution-report",
        "capabilities", "capability-for", "capability-graph", "bypass-audit",
        "bypass-enforcement", "pipeline-enforcement", "scenarios",
        "evidence-integrity", "strengthening-integration",
        "cross-capability-impact", "config-authority-verify", "efficiency",
        "regression", "certify", "latent-audit", "config-authority",
        "generate-test", "c53-scenarios", "c53-certify", "convergence-status",
        "coverage-analysis", "mutation-analysis", "gap-analysis",
        "convergence-plan", "threshold-assessment", "converge", "help-resolve",
    ]
    for token in tokens_to_test:
        cls = classification_for(token)
        assert cls in (
            "CANONICAL", "CANONICAL_ALIAS", "COMPATIBILITY", "DEPRECATED",
            "DUPLICATE", "UNREACHABLE", "TEST-ONLY", "INTERNAL",
...
def test_migration_map_routes_all_legacy_to_canonical():
    ...
    from runtime.foundation.verification.canonical_control_plane import migration_map, classification_for, CanonicalOperation
    ...
    for token, route in migration.items():
        ...
        assert route["canonical_operation"] in canonical_ops, (
...
def test_no_duplicate_authority_legacy_routes():
    ...
    from runtime.foundation.verification.canonical_control_plane import migration_map, classification_for
...
def test_internal_capabilities_remain_importable():
    ...
    from runtime.foundation.verification.canonical_control_plane import (
        CanonicalOperation, canonical_tree, migration_map, classification_for,
    )
    from runtime.foundation.verification.obligation import (
        VerificationObligation, ObligationSet, Disposition, ObligationKind,
        Change, Capability, Requirement, EvidenceRef,
    )
...
def test_obligation_model_has_closed_kind_vocabulary():
    ...
    expected = {"unit", "property", "invariant", "contract", "coverage", "mutation", "golden", "capability"}
...
def test_help_output_contains_inspect_subqueries():
    ...
    result = _run_verify([])
    output = result.stdout + result.stderr
    expected_subqueries = ["capabilities", "evidence", "plan", "mutation", "workflows", "health"]
...
def test_executor_adapters_cover_all_task_kinds():
    ...
    expected_kinds = {"mutation", "unit", "property", "invariant", "contract", "coverage", "golden", "capability"}
    actual_kinds = set(ADAPTERS.keys())
    assert actual_kinds == expected_kinds, f"Missing kinds: {expected_kinds - actual_kinds}"
...
def test_property_invariant_contract_coverage_golden_capability_are_not_executable():
    ...
    for kind in ("property", "invariant", "contract", "coverage", "golden", "capability"):
