...
from runtime.foundation.verification.executor_pipeline import ADAPTERS, VerificationKind
...
class TestFinalCommandGovernance:
    ...
    def test_canonical_command_surface_9_commands(self):
        ...
        assert targets == canonical_ops - {"check"}  # check is primary, not in migration
    ...
    def test_all_legacy_commands_mapped(self):
        ...
        legacy = ["capability-inventory", "forensic-diagnose", "strengthen-analyze",
...
class TestFinalAuthorityGovernance:
    ...
    def test_single_mutation_authority(self):
        ...
        from runtime.foundation.verification.canonical_control_plane import _MIGRATION
...
class TestFinalAdapterGovernance:
    ...
    def test_all_adapters_produce_classified_tasks(self):
        ...
        from runtime.foundation.verification.executor_pipeline import PlannedTask, TaskFingerprints
        for kind, adapter in ADAPTERS.items():
            target = "credit_card_engine" if kind == "mutation" else "test"
            planned = PlannedTask(task_id=f"test-{kind}", task_kind=kind, target=target, disposition="required", cause="test", evidence_id=None, invalidations=(), reuse_disposition=None, notes="")
            fps = TaskFingerprints(source="", test="", config="", toolchain="")
            result = adapter(planned, fps)
            assert result.executable in ("executable", "not_executable_yet"), f"{kind}: {result.executable}"
...
class TestFinalCapabilityGovernance:
    ...
    def test_no_capability_blindness(self):
        ...
        from runtime.foundation.verification.capability_catalog import get_capability_catalog
        from runtime.foundation.verification.capability_graph_resolver import CapabilityGraphResolver, derive_capability_from_path
        catalog = get_capability_catalog()
...
class TestFinalEvidenceGovernance:
    ...
    def test_obligation_reconciliation_gates_certification(self):
        ...
        from runtime.foundation.verification.obligation_reconciliation import reconcile_obligations
        change = Change(path="test.py", change_type="modified")
        obl = VerificationObligation(
            obligation_id="test", change=change,
...
class TestFinalWorkflowGovernance:
    ...
    def test_3_non_verification_workflows_correct(self):
        ...
        non_verification = ["dependency-update.yml", "release.yml", "security-codeql.yml"]
...
class TestFinalDeprecatedPathGovernance:
    ...
    def test_8_deprecated_commands_mapped(self):
        ...
        deprecated = ["capability-inventory", "forensic-diagnose", "strengthen-analyze",
                      "strengthen-discover", "strengthen-propose", "strengthen-validate",
                      "strengthen-report", "mutation-intel"]
        mm = migration_map()
...
class TestFinalArchitectureAcceptanceTests:
    ...
    def test_every_promised_task_classified(self):
        ...
        from runtime.foundation.verification.executor_pipeline import ADAPTERS, PlannedTask, TaskFingerprints
        for kind, adapter in ADAPTERS.items():
            target = "credit_card_engine" if kind == "mutation" else "test"
            planned = PlannedTask(task_id=f"test-{kind}", task_kind=kind, target=target, disposition="required", cause="test", evidence_id=None, invalidations=(), reuse_disposition=None, notes="")
            fps = TaskFingerprints(source="", test="", config="", toolchain="")
    ...
    def test_unmapped_changes_detected(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            ...
            temp_path = f.name
        try:
            unmapped = FileChange(kind=ChangeKind.ADDED, old_path=None, new_path=temp_path)
    ...
    def test_legacy_paths_cannot_bypass(self):
        ...
        mm = migration_map()
        for v in mm.values():
            assert v["canonical_operation"] in {"check", "plan", "run", "diagnose", "strengthen", "inspect", "certify", "ci", "doctor"}
