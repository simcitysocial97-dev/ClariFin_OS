...
def _make_evidence(**overrides):
    ...
    return CIEvidenceRecord(
        ...
        configuration_fingerprint=overrides.get("configuration_fingerprint", "fp-config-default"),
        toolchain_fingerprint=overrides.get("toolchain_fingerprint", "fp-toolchain-default"),
        population_fingerprint=overrides.get("population_fingerprint", "fp-pop-default"),
        evidence_artifact_fingerprint=overrides.get("evidence_artifact_fingerprint", "fp-artifact-default"),
        artifact_path=overrides.get("artifact_path", "runtime/generated/verification-report.md"),
        started_at=overrides.get("started_at", datetime.now(timezone.utc).isoformat()),
        ended_at=overrides.get("ended_at", datetime.now(timezone.utc).isoformat()),
...
def _make_execution_evidence(**overrides):
    ...
    from runtime.foundation.verification.ci_evidence import ExecutionEvidence, FailureKind
    return ExecutionEvidence(
        ...
        started_at=overrides.get("started_at", datetime.now(timezone.utc).isoformat()),
        completed_at=overrides.get("completed_at", datetime.now(timezone.utc).isoformat()),
        ...
        toolchain_fingerprint=overrides.get("toolchain_fingerprint", "fp-toolchain-default"),
        artifact_paths=overrides.get("artifact_paths", ("runtime/generated/verification-report.md",)),
        repository_sha=overrides.get("repository_sha", "abc123"),
...
class TestC55BaselinePreservation(unittest.TestCase):
    ...
    def test_g1_regression_green(self):
        ...
        self.assertEqual(result.returncode, 0, f"C54 regression failed:\n{result.stderr}")
...
class TestC55EnvironmentContract(unittest.TestCase):
    ...
    def test_g5_all_python_tools_match_pins(self):
        from runtime.foundation.verification.env_contract import build_environment_contract

        contract = build_environment_contract()
        tools_to_check = ["pytest", "coverage", "mutmut", "ruff", "black", "mypy", "hypothesis"]
    ...
    def test_g5_npm_matches_pin(self):
        ...
        contract = build_environment_contract()
        self.assertTrue(contract.npm_matches_pin, "npm should match declared packageManager")
...
class TestC55DependencyDeterminism(unittest.TestCase):
    ...
    def test_g3_requirements_lock_reproducible(self):
        data = json.loads((ARTIFACT_DIR / "dependency-reproducibility.json").read_text())
...
class TestC55ToolchainDeterminism(unittest.TestCase):
    ...
    def test_g6_all_tools_documented(self):
        ...
        expected = {"python", "pytest", "coverage", "mutmut", "ruff", "black", "mypy", "hypothesis", "node", "npm"}
    ...
    def test_g6_python_tools_match_pins(self):
        ...
        for tool in data["tools"]:
            if tool["name"] not in skip and tool["pinned_version"] != ">=24 <25 (engine)":
                self.assertTrue(tool["matches_pin"], f"{tool['name']} does not match pin: {tool['resolved_version']}")

    def test_g6_configuration_authority_singular(self):
        from runtime.foundation.verification.configuration_authority import validate_authority
...
class TestC55CoverageEvidenceIntegration(unittest.TestCase):
    ...
    def test_g9_coverage_upload_in_workflow(self):
        from runtime.foundation.verification.workflow_convergence import inventory_workflows
        ...
        has_coverage_step = any(
            "coverage" in (s.name or "").lower() or
            "coverage" in str(s.uses or "")
            for j in mutation_wf.jobs
            for s in j.steps
        )
        self.assertTrue(has_coverage_step, "No coverage-related step found in mutation workflow")

    def test_g10_coverage_fingerprinted(self):
        data = json.loads((ARTIFACT_DIR / "coverage-evidence-contract.json").read_text())
...
class TestC55LocalCIEquivalence(unittest.TestCase):
    ...
    def test_g12_invalidity_causes_incompatible(self):
        ...
        eq = semantic_equivalence(local, ci_record, ingested)
        validity_dim = next((e for e in eq.dimensions if e.dimension == "validity"), None)
...
class TestC55StaleEvidence(unittest.TestCase):
    ...
    def test_g18_stale_coverage_rejected(self):
        from runtime.foundation.verification.ci_evidence import load_ci_evidence, save_ci_evidence
        import tempfile
...
class TestC55ReproducibilityExperiments(unittest.TestCase):
    ...
    def test_g20_backend_repeated_verification(self):
        ...
        result1 = subprocess.run(
            [".venv/bin/python", "-m", "pytest", "runtime/tests/test_m9_c54.py", "-q", "--tb=no"],
            cwd=str(REPO_ROOT),
            capture_output=True,
            text=True,
            timeout=120,
    ...
    def test_g22_coverage_measurement_structure(self):
        ...
        from runtime.foundation.verification.coverage_measurement import measure_coverage_cli
        import tempfile

        with tempfile.TemporaryDirectory() as td:
            out_path = Path(td) / "cov-test.json"
            rc = measure_coverage_cli(["tests/unit/engines/credit_card", "--out", str(out_path)])
            # Infrastructure failures are environmental, not test defects
            self.assertTrue(out_path.exists(), "Coverage output file should be produced")
...
class TestC55DriftExperiments(unittest.TestCase):
    ...
    def test_experiment_f_dependency_drift_detected(self):
        ...
        try:
            ...
            contract_after = build_environment_contract()
            self.assertNotEqual(contract_after.requirements_lock_hash, original_lock_hash)
        finally:
            req_lock.write_text(original_lock)
...
class TestC55ControlPlaneEnforcement(unittest.TestCase):
    ...
    def test_control_plane_allows_consistent_env(self):
        from runtime.foundation.verification.env_contract import build_environment_contract, ContractState

        contract = build_environment_contract()
        self.assertEqual(len(contract.errors), 0)
        self.assertIn(contract.state, [ContractState.CONSISTENT.value, ContractState.WARNINGS.value])
...
class TestC55C53Preservation(unittest.TestCase):
    ...
    def test_g24_c53_modules_intact(self):
        from runtime.foundation.verification.workflow_convergence import verify_c53_integration
    ...
    def test_g24_candidate_validation_intact(self):
        from runtime.foundation.verification.candidate_validation import validate_candidate
        self.assertTrue(callable(validate_candidate))

    def test_g25_authorization_boundary_enforced(self):
        from runtime.foundation.verification.authorization_boundary import build_authorization_report
...
class TestC55ScenarioHarness(unittest.TestCase):
    ...
    def test_scenario_c_reproducible_coverage(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(d1["environment_fingerprint"], d2["environment_fingerprint"])
    ...
    def test_scenario_n_live_ci_unavailable_not_fabricated(self):
        ...
        self.assertTrue(any("GitHub Actions" in l or "github" in l.lower() for l in limitations))
...
class TestC55CertificationGates(unittest.TestCase):
    ...
    def test_g28_no_production_capability_deleted(self):
        ...
        for mod in [ci_evidence, evidence_contract, mutation_contract, coverage_measurement,
                    workflow_convergence, configuration_authority, generation_engine,
                    candidate_validation, authorization_boundary]:
    ...
    def test_g32_certification_derived_from_executable_evidence(self):
        ...
        self.assertIn(contract.state, ["CONSISTENT", "WARNINGS", "INCOMPLETE", "INCOMPATIBLE"])
...
class TestC55ResourceEfficiency(unittest.TestCase):
    ...
    def test_fingerprint_generation_time(self):
        from runtime.foundation.verification.env import resolve_environment, build_fingerprint

        start = time.monotonic()
        env = resolve_environment()
        fp = build_fingerprint(env.python, env.pytest, env.mutmut, list(env.forbidden_venvs))
        elapsed = time.monotonic() - start
...
class TestC55FinalCertification(unittest.TestCase):
    ...
    def test_c56_readiness_assessment(self):
        ...
        (ARTIFACT_DIR / "c56-readiness.json").write_text(json.dumps(assessment, indent=2) + "\n")
