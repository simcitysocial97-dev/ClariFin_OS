...
def check_orphans():
    ...
    result = subprocess.run(
        ["pgrep", "-f", GRANDCHILD_FILE.name], capture_output=True, text=True
    )
...
def test_executor_sigterm_on_cancel():
    ...
    def run_cmd():
        result_container["result"] = executor.execute(
            cmd, task_id="cancel-test", max_retries=0
        )
...
def check_orphans():
    """Check for orphaned grandchild processes."""
    result = subprocess.run(
        ["pgrep", "-f", "m9-c42-14-grandchild"], capture_output=True, text=True
    )
    if result.stdout.strip():
        return result.stdout.strip().split()
    return []
