...
def test_git_fetch_failure():
    ...
    print(
        "NOTE: Current code silently ignores git fetch failures (orchestrator.py:114-116)"
    )
...
def test_editable_install_paths():
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import clarinfin_verification; print(clarinfin_verification.__file__)",
        ],
        capture_output=True,
        text=True,
    )
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            'import sys; print([p for p in sys.path if "clarinfin" in p or ".venv" in p])',
        ],
        capture_output=True,
        text=True,
    )
    ...
    # Check pip show
    result = subprocess.run(
        ["pip", "show", "clarinfin-verification"], capture_output=True, text=True
    )
    print("pip show clarinfin-verification:")
