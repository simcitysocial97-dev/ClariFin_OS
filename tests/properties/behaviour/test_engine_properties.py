...
def invalid_dates(draw: st.DrawFn) -> tuple[str, str]:
    ...
    ref_date = draw(st.dates(min_value=date(2000, 1, 1), max_value=date(2050, 12, 31)))
    last_date = draw(
        st.dates(min_value=ref_date + timedelta(days=1), max_value=date(2050, 12, 31))
    )
    return last_date.isoformat(), ref_date.isoformat()
...
@st.composite
def financial_transactions_with_debt_savings(
    draw: st.DrawFn,
    ...
    for txn in base_transactions:
        if txn["type"] == "debit" and remaining_debt > 0:
            ...
            adjusted_transactions.append({**txn, "amount_paise": txn_amount})
...
def test_financial_stress_extreme_values() -> None:
    ...
    assert (
        0 <= zero_result["score"] <= 1
    ), "Empty transactions should return valid score"
    ...
    assert (
        0 <= minimal_result["score"] <= 1
    ), "Minimal transactions should return valid score"


@given(financial_transactions_with_debt_savings())
def test_financial_stress_monotonicity_savings(
    transactions_and_metrics: tuple[list[dict[str, Any]], int, int],
) -> None:
    ...
    for txn in base_transactions:
        if txn["type"] == "credit":
            lower_savings_transactions.append(
                {**txn, "amount_paise": txn["amount_paise"] // 2}  # Halve savings
            )
        else:
    ...
    assert (
        lower_savings_result["eom_depletion_ratio"]
        >= base_result["eom_depletion_ratio"] - 0.001
...
@given(financial_transactions())
def test_financial_stress_deterministic_output(
...
def test_financial_stress_credit_dependency(transactions: list[dict[str, Any]]) -> None:
    ...
    if not any(txn["type"] == "debit" for txn in transactions) or not any(
        txn["type"] == "credit" for txn in transactions
    ):
    ...
    for txn in transactions:
        if txn["type"] == "credit":
            high_credit_transactions.append(
                {**txn, "amount_paise": txn["amount_paise"] * 2}  # Double credit
            )
        else:
    ...
    assert (
        high_credit_result["credit_dependency"]
        >= base_result["credit_dependency"] - 0.001
...
def test_financial_stress_invalid_inputs(transactions: list[dict[str, Any]]) -> None:
    ...
    assert (
        empty_result["score"] == 0.5
    ), "Empty transactions should return default score"

    # Test missing required fields
    invalid_txn = [{"type": "debit"}]  # Missing amount_paise and date_iso
    invalid_result = financial_stress_index(invalid_txn)
    assert (
        0 <= invalid_result["score"] <= 1
    ), "Invalid transactions should return valid score"
...
def test_financial_stress_edge_cases(transactions: list[dict[str, Any]]) -> None:
    ...
    if zero_debt_transactions:
        ...
        assert (
            0 <= zero_debt_result["score"] <= 1
        ), "Zero debt should return valid score"
    ...
    for txn in transactions:
        if txn["type"] == "credit":
            max_credit_transactions.append(
                {**txn, "amount_paise": txn["amount_paise"] * 10}  # Max credit
            )
        else:
