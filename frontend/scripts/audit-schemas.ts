...
const openApiSchema = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../generated/openapi-current.json"), "utf-8")
)

// Map DTO names to Zod schemas
const dtoToZodMap: Record<string, { file: string; schemaName: string }> = {
...
const isMonetaryField = (fieldName: string): boolean => fieldName.endsWith("_paise")

// Helper: Validate Zod schema against DTO
const validateSchema = (dtoName: string, dtoSchema: any): { status: string; mismatches: string[] } => {
  ...
  for (const [fieldName, _] of Object.entries(dtoFields)) {
...
// Audit all DTOs
const auditResults: Record<string, { status: string; mismatches: string[] }> = {}

for (const [_path, methods] of Object.entries(openApiSchema.paths)) {
  for (const [_method, spec] of Object.entries(methods as Record<string, any>)) {
