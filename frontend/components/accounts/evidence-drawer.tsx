/**
 * Accounts Evidence Drawer - Stage 4 Accounts Intelligence Workspace
 *
 * Shows explainability evidence for account calculations.
 *
 * Architecture Flow: Backend → API → DTO → Mapper → ViewModel → Capability → Workspace → Components → Page
 */

import { X, FileText, Calculator, Link } from 'lucide-react';
import type { AccountsViewModel, AccountEvidenceChainViewModel } from '@/types/accounts-view-model';

/**
 * Accounts Evidence Drawer Props
 */
...
/**
 * Accounts Evidence Drawer Component
 */
export function EvidenceDrawer({ accounts, isOpen, onClose }: EvidenceDrawerProps) {
  if (!isOpen) return null;

  const evidenceChain: AccountEvidenceChainViewModel | undefined = accounts?.evidence_chain;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-label="Accounts evidence drawer"
      >
        <div className="p-4">
