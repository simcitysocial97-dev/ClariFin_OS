/**
 * Forecast Evidence Drawer - Stage 4 Forecast Intelligence Workspace
 *
 * Shows explainability evidence for forecast calculations.
 *
 * Architecture Flow: Backend → API → DTO → Mapper → ViewModel → Capability → Workspace → Components → Page
 */

import { X, FileText, Calculator, Link } from 'lucide-react';
import type { ForecastViewModel, ForecastEvidenceChainViewModel } from '@/types/forecast-view-model';

/**
 * Forecast Evidence Drawer Props
 */
...
/**
 * Forecast Evidence Drawer Component
 */
export function EvidenceDrawer({ forecast, isOpen, onClose }: EvidenceDrawerProps) {
  if (!isOpen) return null;

  const evidenceChain: ForecastEvidenceChainViewModel | undefined = forecast?.evidence_chain;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-label="Forecast evidence drawer"
      >
        <div className="p-4">
