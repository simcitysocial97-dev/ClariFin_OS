/**
 * Loans Evidence Drawer - Stage 4 Loans Intelligence Workspace
 *
 * Shows explainability evidence for loan calculations.
 *
 * Architecture Flow: Backend → API → DTO → Mapper → ViewModel → Capability → Workspace → Components → Page
 */

import { X, FileText, Calculator, Link } from 'lucide-react';
import type { LoansViewModel, LoanEvidenceChainViewModel } from '@/types/loans-view-model';

/**
 * Loans Evidence Drawer Props
 */
...
/**
 * Loans Evidence Drawer Component
 */
export function EvidenceDrawer({ loans, isOpen, onClose }: EvidenceDrawerProps) {
  if (!isOpen) return null;

  const evidenceChain: LoanEvidenceChainViewModel | undefined = loans?.evidence_chain;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-label="Loans evidence drawer"
      >
        <div className="p-4">
