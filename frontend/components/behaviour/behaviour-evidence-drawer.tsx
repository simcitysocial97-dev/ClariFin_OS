/**
 * Behaviour Evidence Drawer - Stage 4 Behaviour Intelligence Workspace
 *
 * Shows explainability evidence for behaviour calculations.
 *
 * Architecture Flow: Backend → API → DTO → Mapper → ViewModel → Capability → Workspace → Components → Page
 */

import { X, FileText, Calculator, Link } from 'lucide-react';
import type { BehaviourViewModel, BehaviourEvidenceChainViewModel } from '@/types/behaviour-view-model';

/**
 * Behaviour Evidence Drawer Props
 */
...
/**
 * Behaviour Evidence Drawer Component
 */
export function EvidenceDrawer({ behaviour, isOpen, onClose }: EvidenceDrawerProps) {
  if (!isOpen) return null;

  const evidenceChain: BehaviourEvidenceChainViewModel | undefined = behaviour?.evidence_chain;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-label="Behaviour evidence drawer"
      >
        <div className="p-4">
