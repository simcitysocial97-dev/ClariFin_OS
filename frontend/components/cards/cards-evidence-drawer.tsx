/**
 * Credit Cards Evidence Drawer - Stage 4 Credit Cards Intelligence Workspace
 *
 * Shows explainability evidence for credit card calculations.
 *
 * Architecture Flow: Backend → API → DTO → Mapper → ViewModel → Capability → Workspace → Components → Page
 */

import { X, FileText, Calculator, Link } from 'lucide-react';
import type { CreditCardsViewModel, CreditCardEvidenceChainViewModel } from '@/types/credit-cards-view-model';

/**
 * Credit Cards Evidence Drawer Props
 */
interface EvidenceDrawerProps {
  cards: CreditCardsViewModel | null;
  isOpen: boolean;
  onClose: () => void;
}
...
/**
 * Credit Cards Evidence Drawer Component
 */
export function EvidenceDrawer({ cards, isOpen, onClose }: EvidenceDrawerProps) {
  if (!isOpen) return null;

  const evidenceChain: CreditCardEvidenceChainViewModel | undefined = cards?.evidence_chain;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-label="Credit cards evidence drawer"
      >
        <div className="p-4">
