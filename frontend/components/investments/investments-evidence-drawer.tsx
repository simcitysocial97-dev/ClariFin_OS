/**
 * Investments Evidence Drawer - Stage 4 Investments Intelligence Workspace
 *
 * Shows explainability evidence for investment calculations.
 *
 * Architecture Flow: Backend → API → DTO → Mapper → ViewModel → Capability → Workspace → Components → Page
 */

import { X, FileText, Calculator, Link } from 'lucide-react';
import type { InvestmentsViewModel, InvestmentEvidenceChainViewModel } from '@/types/investments-view-model';

/**
 * Investments Evidence Drawer Props
 */
...
/**
 * Investments Evidence Drawer Component
 */
export function EvidenceDrawer({ investments, isOpen, onClose }: EvidenceDrawerProps) {
  if (!isOpen) return null;

  const evidenceChain: InvestmentEvidenceChainViewModel | undefined = investments?.evidence_chain;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-label="Investments evidence drawer"
      >
        <div className="p-4">
