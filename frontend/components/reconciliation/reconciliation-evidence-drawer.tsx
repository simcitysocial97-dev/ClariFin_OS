/**
 * Reconciliation Evidence Drawer - Stage 4 Reconciliation Intelligence Workspace
 *
 * Shows explainability evidence for reconciliation calculations.
 *
 * Architecture Flow: Backend → API → DTO → Mapper → ViewModel → Capability → Workspace → Components → Page
 */

import { X, FileText, Calculator, Link } from 'lucide-react';
import type { ReconciliationViewModel, ReconciliationEvidenceChainViewModel } from '@/types/reconciliation-view-model';

/**
 * Reconciliation Evidence Drawer Props
 */
...
/**
 * Reconciliation Evidence Drawer Component
 */
export function EvidenceDrawer({ reconciliation, isOpen, onClose }: EvidenceDrawerProps) {
  if (!isOpen) return null;

  const evidenceChain: ReconciliationEvidenceChainViewModel | undefined = reconciliation?.evidence_chain;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-label="Reconciliation evidence drawer"
      >
        <div className="p-4">
