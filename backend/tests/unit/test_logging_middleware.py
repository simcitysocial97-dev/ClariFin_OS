...
def test_transactions_endpoint_logs_request(
    ...
    matching = [
        ...
        if "GET /api/v1/transactions - 200" in r.getMessage()
...
def test_correlation_id_passthrough(client: TestClient) -> None:
    ...
    response = client.get(
        "/api/v1/transactions", headers={"X-Correlation-Id": "m07-test-id"}
    )
...
def test_status_code_reflected_in_log(
    ...
    with caplog.at_level(logging.INFO, logger="clarifin"):
        response = client.get("/api/v1/does-not-exist-m07")

    assert response.status_code == 404
    assert any(
        "GET /api/v1/does-not-exist-m07 - 404" in r.getMessage() for r in caplog.records
    )
