...
from src.engines.loan_engine.models import AmortizationRow, PrepaymentMode, PrepaymentResult
...
def prepayment_parameters(draw):
    ...
    prepayment_amount = draw(st.integers(min_value=MIN_PREPAYMENT_PAISE, max_value=min(MAX_PREPAYMENT_PAISE, principal)))
...
def test_apply_prepayment_invariants(prepayment_params):
    ...
    principal, rate, tenure, prepayment_amount, prepayment_month, start_date = prepayment_params

    # Apply prepayment
    result = apply_prepayment(
        principal, rate, tenure, prepayment_amount, PrepaymentMode.REDUCE_TENURE, start_date
    )
...
def test_apply_prepayment_at_month_reduce_emi_mode(schedule_params):
    ...
    new_schedule, result = apply_prepayment_at_month(
        schedule, prepayment_month, prepayment_amount, rate, mode=PrepaymentMode.REDUCE_EMI
    )
...
def test_apply_prepayment_at_month_reduce_tenure_mode(schedule_params):
    ...
    new_schedule, result = apply_prepayment_at_month(
        schedule, prepayment_month, prepayment_amount, rate, mode=PrepaymentMode.REDUCE_TENURE
    )
...
def test_regenerate_schedule_math_accuracy(principal, rate, tenure, start_date):
    ...
    for new_row, original_row in zip(new_schedule, original_schedule):
