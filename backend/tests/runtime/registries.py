"""Unified registry loaders for the Verification Runtime.

Wraps existing generated artifacts (capability-registry.yaml, contract-registry.json,
mutation-registry.json, api-map.json) into a single source of truth.

All paths are relative to backend/tests/generated/.
"""

from __future__ import annotations

import json
from pathlib import Path
from typing import Any

# Generated artifacts directory
GENERATED_DIR = Path(__file__).parent.parent / "generated"


def load_capability_registry() -> dict[str, Any]:
    """Load capability-registry.yaml.

    Returns dict with 'capabilities' list.
    Each capability has: id, name, description, criticality, risk,
    routers, services, engines, repositories, tables, golden_datasets,
    property_tests, invariants, architecture_tests, contracts, dependencies, failure_impact.
    """
    import yaml

    path = GENERATED_DIR / "capability-registry.yaml"
    if not path.exists():
        return {"capabilities": []}
    with open(path) as f:
        data = yaml.safe_load(f) or {"capabilities": []}
    return data


def load_contract_registry() -> dict[str, Any]:
    """Load contract-registry.json.

    Returns dict with 'routers' and 'generated_at'.
    Each router has: endpoints list with method, path, request_schema, response_schema, status_codes.
    """
    path = GENERATED_DIR / "contract-registry.json"
    if not path.exists():
        return {"routers": {}, "generated_at": None}
    with open(path) as f:
        return json.load(f)


def load_api_map() -> dict[str, Any]:
    """Load api-map.json with full endpoint metadata.

    Returns dict with 'endpoints' list and 'generated_at'.
    Each endpoint has: router, endpoint, method, tags, capability, request_schema, response_schema, status_codes, parameters.
    """
    path = GENERATED_DIR / "api-map.json"
    if not path.exists():
        return {"endpoints": [], "generated_at": None}
    with open(path) as f:
        return json.load(f)


def load_mutation_registry() -> dict[str, Any]:
    """Load mutation-registry.json.

    Returns dict with 'generated_at' and 'entries' list.
    Each entry has: module, function, capability, risk, mutation_types, existing_tests, property_tests, golden_tests, contracts, performance_tests.
    """
    path = GENERATED_DIR / "mutation-registry.json"
    if not path.exists():
        return {"generated_at": None, "entries": []}
    with open(path) as f:
        return json.load(f)


def load_validation_manifest() -> dict[str, Any] | None:
    """Load validation-manifest.json from last run.

    Returns dict or None if not present.
    """
    path = GENERATED_DIR / "validation-manifest.json"
    if not path.exists():
        return None
    with open(path) as f:
        return json.load(f)


def load_risk_rules() -> dict[str, Any]:
    """Load risk-rules.yaml.

    Returns dict with 'rules' list.
    Each rule has: pattern, strategy, risk, reason.
    """
    import yaml

    path = GENERATED_DIR / "risk-rules.yaml"
    if not path.exists():
        return {"rules": []}
    with open(path) as f:
        return yaml.safe_load(f) or {"rules": []}


def get_all_capability_ids() -> list[str]:
    """Get all registered capability IDs."""
    registry = load_capability_registry()
    return [
        cap.get("id", "") for cap in registry.get("capabilities", []) if cap.get("id")
    ]


def get_capability_by_id(capability_id: str) -> dict[str, Any] | None:
    """Get a single capability by ID."""
    registry = load_capability_registry()
    for cap in registry.get("capabilities", []):
        if cap.get("id") == capability_id:
            return cap
    return None


def get_endpoints_by_capability(capability_id: str) -> list[dict[str, Any]]:
    """Get all API endpoints belonging to a capability."""
    api_map = load_api_map()
    return [
        ep
        for ep in api_map.get("endpoints", [])
        if ep.get("capability") == capability_id
    ]


def get_mutation_entries_by_capability(capability_id: str) -> list[dict[str, Any]]:
    """Get all mutation entries for a capability."""
    registry = load_mutation_registry()
    return [
        entry
        for entry in registry.get("entries", [])
        if entry.get("capability") == capability_id
    ]


def load_dependency_map() -> dict[str, Any]:
    """Load dependency-map.json from the intelligence layer.

    Returns dict with 'edges' list and 'capabilities' dict.
    """
    path = GENERATED_DIR / "dependency-map.json"
    if not path.exists():
        return {"edges": [], "capabilities": {}, "generated_at": None}
    with open(path) as f:
        return json.load(f)


def load_change_impact() -> dict[str, Any]:
    """Load change-impact.json from the intelligence layer.

    Returns dict with impact analysis results.
    """
    path = GENERATED_DIR / "change-impact.json"
    if not path.exists():
        return {
            "changed_files": [],
            "affected_capabilities": [],
            "overall_risk": "LOW",
            "strategy": "full",
            "generated_at": None,
        }
    with open(path) as f:
        return json.load(f)


def load_verification_evidence() -> dict[str, Any]:
    """Load verification-evidence.json from the intelligence layer.

    Returns dict with evidence summaries for each capability.
    """
    path = GENERATED_DIR / "verification-evidence.json"
    if not path.exists():
        return {
            "capabilities": [],
            "total_capabilities": 0,
            "fully_verified": 0,
            "partial": 0,
            "missing": 0,
            "generated_at": None,
        }
    with open(path) as f:
        return json.load(f)


def load_architectural_coverage() -> dict[str, Any]:
    """Load architectural-coverage.json from the intelligence layer.

    Returns dict with coverage status for each capability.
    """
    path = GENERATED_DIR / "architectural-coverage.json"
    if not path.exists():
        return {"capabilities": [], "summary": {}, "generated_at": None}
    with open(path) as f:
        return json.load(f)


def load_risk_map() -> dict[str, Any]:
    """Load risk-map.json from the intelligence layer.

    Returns dict with risk classifications for all components.
    """
    path = GENERATED_DIR / "risk-map.json"
    if not path.exists():
        return {"entries": [], "generated_at": None}
    with open(path) as f:
        return json.load(f)


def load_verification_summary() -> dict[str, Any]:
    """Load verification-summary.json from the intelligence layer.

    Returns combined summary of all intelligence reports.
    """
    path = GENERATED_DIR / "verification-summary.json"
    if not path.exists():
        return {"reports": {}, "generated_at": None}
    with open(path) as f:
        return json.load(f)
