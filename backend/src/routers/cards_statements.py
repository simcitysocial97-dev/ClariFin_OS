...
def get_cards() -> dict[str, Any]:
    ...
    for (bank, card_last4), statements in card_groups.items():
        ...
        days_until_due = None
        if payment_due_date:
            # parse_date() returns None (never raises) for unparseable
            # input, so no try/except is needed; a None result keeps
            # days_until_due as None and payment_status as "unknown".
