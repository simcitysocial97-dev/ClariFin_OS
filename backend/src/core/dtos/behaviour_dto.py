...
class WellnessRadarDTO(BaseModel):
    ...
    dimension: str = Field(description="Dimension name (e.g., 'Savings', 'Debt', 'Income')")
    score: int = Field(description="Score in basis points (0-10000)")
...
class BehaviourDTO(BaseModel):
    ...
    savings_rate: SavingsRateDTO | None = Field(
        default=None,
        description="Savings rate analysis"
    )
    debt_health: DebtHealthDTO | None = Field(
        default=None,
        description="Debt health analysis"
    )
    wellness_radar: list[WellnessRadarDTO] = Field(
        default_factory=list,
        description="Wellness radar data"
    )
    insights: list[BehaviourInsightDTO] = Field(
        default_factory=list,
        description="List of insights about behaviour"
    )
...
class BehaviourSavingsResponse(BaseModel):
    ...
    savings_rate: SavingsRateDTO = Field(
        default_factory=lambda: SavingsRateDTO(savings_rate_bps=0, income_paise=0, savings_paise=0, period="1M"),
...
class BehaviourDebtResponse(BaseModel):
    ...
    debt_health: DebtHealthDTO = Field(
        default_factory=lambda: DebtHealthDTO(debt_to_income_bps=0, total_debt_paise=0, total_income_paise=0, health_score=0),
