...
class NetWorthBreakdownItemDTO(BaseModel):
    ...
    type: str = Field(description="Account type (savings, current, investment, loan, credit_card)")
    balance_paise: int = Field(description="Balance in paise (can be negative for liabilities)")
...
class NetWorthHistoricalSnapshotDTO(BaseModel):
    ...
    liabilities_paise: int = Field(description="Total liabilities in paise on this date")
...
class NetWorthTrendDTO(BaseModel):
    ...
    direction: NetWorthTrendDirection = Field(description="Trend direction (up/down/flat)")
...
class NetWorthDTO(BaseModel):
    ...
    """
    total_net_worth_paise: int = Field(description="Net worth in paise (assets - liabilities)")
    total_assets_paise: int = Field(description="Total assets in paise")
    total_liabilities_paise: int = Field(description="Total liabilities in paise")
...
class NetWorthHistoryResponse(BaseModel):
    ...
    snapshots: list[NetWorthHistoricalSnapshotDTO] = Field(
        default_factory=list,
        description="Historical net worth snapshots"
    )
...
class NetWorthInsightsResponse(BaseModel):
    ...
    insights: list[NetWorthInsightDTO] = Field(
        default_factory=list,
        description="List of net worth insights"
    )
