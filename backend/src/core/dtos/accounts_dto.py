...
class AccountDetailDTO(BaseModel):
    ...
    opened_date: str | None = Field(
        default=None, description="Account opening date (ISO)"
    )
...
class AccountsDTO(BaseModel):
    ...
    accounts: list[AccountDetailDTO] = Field(
        default_factory=list, description="List of account details"
    )
    ...
    type_breakdown: list[AccountTypeBreakdownDTO] = Field(
        default_factory=list, description="Account type breakdown"
    )
    insights: list[AccountInsightDTO] = Field(
        default_factory=list, description="List of insights about accounts"
    )
...
class AccountsHistoryResponse(BaseModel):
    ...
    history: list[BalanceHistoryDTO] = Field(
        default_factory=list, description="Balance history entries"
    )
