...
def find_value_near(
    text: str, label: str, value_type: str = "currency", max_distance: int = 200
) -> Any | None:
    """
...
BANK_CONFIGS: dict[str, dict[str, Any]] = {
    "HDFC Bank": {
        ...
        "opening_balance": {
            "direct_pattern": re.compile(
                r"Opening\s*Balance[\s\S]*?([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "bill_cycle": {
            "pattern": re.compile(
                r"Statement Date[:\s]+(\d{2}/\d{2}/\d{4})", re.IGNORECASE
            ),
        },
    },
    "Axis Bank": {
        ...
        "credit_limit": {
            "direct_pattern": re.compile(
                r"Credit Limit[\s\S]*?([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "opening_balance": {
            "direct_pattern": re.compile(
                r"Previous Balance[\s\S]*?([\d,]+\.\d{2})\s+Dr", re.IGNORECASE
            ),
        },
        "bill_cycle": {
    ...
    "ICICI Bank": {
        ...
        "total_amount_due": {
            ...
            "direct_pattern": re.compile(
                r"Total Amount due[\s\S]*?`\s*([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "minimum_amount_due": {
            # Anchor to "Minimum Amount due" label to get correct value
            "direct_pattern": re.compile(
                r"Minimum Amount due[\s\S]*?`\s*([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "due_date": {
            "direct_pattern": re.compile(
                r"PAYMENT DUE DATE[\s\S]*?(\w+\s+\d{1,2},\s*\d{4})", re.IGNORECASE
            ),
        },
        "credit_limit": {
            "direct_pattern": re.compile(
                r"Credit Limit[\s\S]*?`\s*(\d{1,2},\d{2},\d{3}\.\d{2})", re.IGNORECASE
            ),
        },
        "opening_balance": {
            "direct_pattern": re.compile(
                r"Previous Balance[\s\S]*?`\s*([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "bill_cycle": {
            "pattern": re.compile(
                r"STATEMENT DATE[\s\S]*?(\w+\s+\d{1,2},\s*\d{4})", re.IGNORECASE
            ),
        },
    },
    "SBI Card": {
        "card_number": {
            "direct_pattern": re.compile(r"XXXX XXXX XXXX XX(\d{2})", re.IGNORECASE),
            "transform": lambda m: "XXXX XXXX XXXX XX" + m,
        },
        "total_amount_due": {
            # SBI: "*Total Amount Due ( ` )" on one line, value on next line
            "direct_pattern": re.compile(
                r"\*Total Amount Due[\s\S]*?([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "minimum_amount_due": {
            # SBI: "**Minimum Amount Due( ` )" then CKYC line with value
            "direct_pattern": re.compile(
                r"CKYC No[\s\S]*?:\s*\d+\s+([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "due_date": {
            # SBI: "Payment Due Date" on one line, date on next line
            "direct_pattern": re.compile(
                r"Payment Due Date[\s\S]*?(\d{1,2}\s+\w{3}\s+\d{4})", re.IGNORECASE
            ),
        },
        "credit_limit": {
            "direct_pattern": re.compile(
        ...
        "opening_balance": {
            "direct_pattern": re.compile(
                r"Previous Balance[\s\S]*?([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "bill_cycle": {
            "pattern": re.compile(
    ...
    "IDFC First Bank": {
        "card_number": {
            "direct_pattern": re.compile(r"FIRST\s+\w+\+?\s+(XX\d{4})", re.IGNORECASE),
        },
        "total_amount_due": {
            "direct_pattern": re.compile(
                r"Total Amount Due[\s\S]*?r\s*([\d,]+\.\d{2})\s*DR", re.IGNORECASE
            ),
        },
        "minimum_amount_due": {
            "direct_pattern": re.compile(
                r"Minimum Amount Due[\s\S]*?r\s*([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "due_date": {
            "direct_pattern": re.compile(
                r"Payment Due Date[\s\S]*?(\d{2}/\w{3}/\d{4})", re.IGNORECASE
            ),
        },
        "credit_limit": {
            "direct_pattern": re.compile(
                r"Credit Limit[\s\S]*?r\s*([\d,]+)", re.IGNORECASE
            ),
        },
        "opening_balance": {
            "direct_pattern": re.compile(
                r"Opening Balance[\s\S]*?r\s*([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "bill_cycle": {
            "pattern": re.compile(
                r"(\d{2}/\w{3}/\d{4})\s*-\s*(\d{2}/\w{3}/\d{4})", re.IGNORECASE
            ),
        },
    },
    "IndusInd Bank": {
        ...
        "total_amount_due": {
            ...
            "direct_pattern": re.compile(
                r"Total Amount Due[\s\S]*?([\d,]+\.\d{2})\s*DR", re.IGNORECASE
            ),
        },
        "minimum_amount_due": {
            # IndusInd: "Minimum Amount Due" on one line, value on next line
            "direct_pattern": re.compile(
                r"Minimum Amount Due[\s\S]*?([\d,]+\.\d{2})", re.IGNORECASE
            ),
        },
        "due_date": {
            # IndusInd: "Payment Due Date" on one line, date on next line
            "direct_pattern": re.compile(
                r"Payment Due Date[\s\S]*?(\d{2}/\d{2}/\d{4})", re.IGNORECASE
            ),
        },
        "credit_limit": {
            "direct_pattern": re.compile(
                r"Credit Limit[\s\S]*?([\d,]+)", re.IGNORECASE
            ),
        },
        "opening_balance": {
            "direct_pattern": re.compile(
        ...
        "bill_cycle": {
            "pattern": re.compile(
                r"(\d{2}/\d{2}/\d{4})\s*To\s*(\d{2}/\d{2}/\d{4})", re.IGNORECASE
            ),
        },
    },
}
    ...
    def extract(self) -> dict[str, Any]:
        ...
        if not config:
            ...
            for bank_name, bank_config in BANK_CONFIGS.items():
                ...
                test_total = self._extract_field(
                    "total_amount_due", bank_config.get("total_amount_due", {})
                )
                ...
                if pattern:
                    ...
                    if match:
                        if match.lastindex and match.lastindex >= 2:
                            ...
                            self._log(
                                f"  bill_cycle: {match.group(1)} to {match.group(2)}"
                            )
                        else:
