...
class BankRepository(BaseRepository):
    ...
    def create(self, name: str, metadata: Optional[dict[str, Any]] = None) -> dict[str, Any]:
        ...
        with self._get_conn() as conn:
            ...
            conn.commit()
            result = self.get_by_id(cur.lastrowid)
            if result is None:
                raise ValueError("Failed to retrieve created bank record")
            return result

    def update(self, bank_id: int, name: Optional[str] = None, metadata: Optional[dict[str, Any]] = None) -> Optional[dict[str, Any]]:
        """Update bank details."""
        with self._get_conn() as conn:
            if name is not None:
                conn.execute("UPDATE banks SET name = ? WHERE id = ?", (name, bank_id))
            if metadata is not None:
                conn.execute("UPDATE banks SET metadata = ? WHERE id = ?", (metadata, bank_id))
            conn.commit()
