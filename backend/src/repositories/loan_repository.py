...
class LoanRepository(BaseRepository):
    ...
    def create(self, name: str, lender: str, loan_type: str, principal_paise: int,
               ...
               interest_type: str = 'reducing',
               notes: str | None = None) -> int:
        """Legacy method - use create_loan() instead."""
        return self.create_loan(
            name, lender, loan_type, principal_paise,
            outstanding_paise, interest_rate, disbursed_date, tenure_months,
            emi_paise, next_emi_date, gold_weight_grams, gold_purity,
            interest_type, notes
        )

    def get_by_id(self, loan_id: int | str) -> dict[str, Any] | None:
        """Legacy method - use get_loan() instead."""
        return self.get_loan(loan_id)

    def get_all(self) -> list[dict[str, Any]]:
        """Legacy method - use list_loans() instead."""
        return self.list_loans()

    def update(self, loan_id: int | str, **kwargs: str | int | float | None) -> dict[str, Any] | None:
        """Legacy method - use update_loan() instead."""
        return self.update_loan(loan_id, **kwargs)

    def delete(self, loan_id: int | str) -> bool:
        """Legacy method - use delete_loan() instead."""
        return self.delete_loan(loan_id)
