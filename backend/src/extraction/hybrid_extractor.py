...
def _pdfplumber_extract(pdf_path: str, page_num: int, bbox: Tuple,
    ...
    try:
        ...
        if debug:
            print(f"[HybridExtractor] pdfplumber fallback error on page {page_num}: {e}")
...
class HybridExtractor:
    ...
    HEADER_KEYWORDS = [
        ...
        'fx transactions', 'particulars', 'narration', 'withdrawal',
        'deposit', 'txn date', 'txn amount',
    ]
    ...
    def _extract_page_camelot(self, page_num: int, bbox: Tuple,
        ...
        for attempt in attempts:
            try:
                ...
                self._log(f"Page {page_num}: {attempt['flavor']} (cols={attempt['use_cols']}) found {len(tables)} tables")
    ...
    def _post_process_rows(self, raw_rows: List[List[str]],
        ...
        for row in raw_rows:
            ...
            if _row_has_date(first_cell):
                ...
                pending = self._build_transaction(cell, col_names, amount_structure, row)
            else:
    ...
    def _build_transaction(self, cell: Dict, col_names: List[str],
        ...
        used_keys = {col_names[0], desc_key,
    ...
    def _find_key(self, col_names: List[str], candidates: List[str]) -> Optional[str]:
    ...
    def _find_description_key(self, col_names: List[str]) -> Optional[str]:
        return self._find_key(col_names, [
            'description', 'narration', 'particulars', 'details', 'col_1'
        ])
    ...
    def _infer_columns_from_rows(self, raw_rows: List[List[str]],
        ...
        self._log(f"Column count mismatch: Camelot={most_common_width}, LayoutAnalyzer={layout_width} → using inferred columns")
    ...
    def extract(self) -> Dict:
        ...
        self._log(f"Bank: {bank}, Pages: {table_pages}, Columns: {list(columns.keys())}")
        ...
        with pdfplumber.open(self.pdf_path) as pdf:
            for page_num in table_pages:
                ...
                if rows:
                    ...
                    self._log(f"Page {page_num}: Camelot returned 0 rows → pdfplumber fallback")
                    fallback_rows = _pdfplumber_extract(
                        self.pdf_path, page_num, bbox, columns, self.debug
                    )
                    self._log(f"Page {page_num}: pdfplumber fallback got {len(fallback_rows)} rows")
        ...
        self._log(f"Effective columns for post-processing: {list(effective_columns.keys())}")
