...
def seed():
    ...
    bank_resp = client.post(
        f"{BASE}/api/banks", json={"name": "Test Bank", "currency": "INR"}
    )
...
"""
    ...
    if detect_resp.status_code == 200:
        ...
        if execute_resp.status_code in (200, 201):
            ...
            print(
                f"  Import execute failed: {execute_resp.status_code} {execute_resp.text[:100]}"
            )
    else:
    ...
    if txn_resp.status_code == 200:
        txns = txn_resp.json()
        print(
            f"  Verified: {len(txns) if isinstance(txns, list) else 'unknown'} transactions in DB"
        )
